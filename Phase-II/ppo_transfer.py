...
def parse_args():
    ...
    parser.add_argument("--attribute", type = str, default = "force_mag", help="the attribute to be changed")

    # Algorithm specific arguments
    parser.add_argument("--env-id", type=str, default="CartPole-v1",
        help="the id of the environment")
    parser.add_argument("--total-timesteps", type=int, default=512000,
...
class Agent(nn.Module):
    """
        Consists of an actor and a critic
...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"{args.env_id}__{args.exp_name}__{args.attribute}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    for update in range(1, num_updates + 1):
        ...
        if update == update_points[update_idx] + 1:
            print(f"Changing {args.attribute} from {getattr(envs.envs[0], args.attribute)} to {update_values[update_idx]}")
            
            for env in envs.envs:
                setattr(env, args.attribute, update_values[update_idx])
